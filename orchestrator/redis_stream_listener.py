...
# This works for both host execution and Docker if the parent is /app
project_root = str(Path(__file__).resolve().parent.parent)
if project_root not in sys.path:
    sys.path.insert(0, project_root)

# Also keep /app for backward compatibility in K8s/Docker environments
...
class RedisStreamListener:
    ...
    async def _deploy_agent_container(self, agent_name: str, image_tag: str, owner_id: str, upload_type: Optional[str] = None, webhook_url: Optional[str] = None) -> dict:
        ...
        docker_cmd = [
            ...
            '-v', f"{Config.HOST_PROJECT_PATH}/.nasiko-local.env:/app/.env:ro" # Unified config: Mount host env
        ]
