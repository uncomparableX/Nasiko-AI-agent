...
def register_service_in_kong(service: ServiceInfo) -> bool:
    ...
    try:
        ...
        route_data = {
            ...
            "strip_path": True,
            "preserve_host": False,
            "regex_priority": 100  # Ensure agent routes take precedence over landing page
        }
        ...
        # Apply default plugins for agents
        try:
            apply_middlewares_to_route(f"{service.name}-route", ["cors", "chat-logger"])
        except Exception as e:
            logger.warning(f"Failed to apply middlewares to {service.name}-route: {e}")
...
def register_static_proxies():
    ...
    auth_host = _resolve_service_host(
        ...
        local_service="nasiko-auth-service",
    ...
    static_services = [
        # Backend API proxy - auth only, no chat logging
        {
            ...
            "preserve_host": False,
            "middlewares": ["cors"]  # CORS + Auth, no chat logging
        },
        ...
        # Router service - mapped to /router path
        {
            ...
            "preserve_host": False,
            "middlewares": ["cors", "chat-logger"]  # Full middleware with CORS
        },
        ...
        try:
            ...
            for service in services:
                if register_service_in_kong(service):
                    ...
                    # Note: We temporarily remove nasiko-auth for demo app compatibility
                    route_name = f"{service.name}-route"
                    apply_middlewares_to_route(route_name, ["cors", "chat-logger"])
